...
def _import_csv_file(
    ...
    try:
        ...
        if replace and (prune_missing is None or prune_missing):
            ...
            stale_ids = [
                game.id for game in existing if game.source_game_id not in seen_gameids
            ]
            for offset in range(0, len(stale_ids), 500):
                chunk = stale_ids[offset : offset + 500]
                session.exec(
                    delete(LolPlayerGameStat).where(
                        LolPlayerGameStat.game_id.in_(chunk)
                    )
                )
                session.exec(
                    delete(LolTeamGameStat).where(LolTeamGameStat.game_id.in_(chunk))
                )
...
def _process_game(
    session: Session, gameid: str, rows: list[dict], replace: bool = False
):
    source_key = f"oracles_elixir:{gameid}"
    existing = session.exec(
        select(LolGameHistory).where(LolGameHistory.source_key == source_key)
    ).first()
    ...
    for row in rows:
        position = (
            (row.get("position") or row.get("participantid") or "").strip().lower()
        )
        (team_rows if position in ("", "team", "total", "0") else player_rows).append(
            row
        )
    if not team_rows:
        return {"games": 0, "teams": 0, "players": 0}
    ...
    if existing:
        session.exec(
            delete(LolPlayerGameStat).where(LolPlayerGameStat.game_id == existing.id)
        )
        session.exec(
            delete(LolTeamGameStat).where(LolTeamGameStat.game_id == existing.id)
        )
        game = existing
    else:
        game = LolGameHistory(
            source_name="oracles_elixir", source_game_id=gameid, source_key=source_key
        )
    ...
    game.winner_team = next(
        (
            (row.get("teamname") or "").strip()
            for row in team_rows
            if _int(row.get("result")) == 1
        ),
        None,
    )
    session.add(game)
    ...
    for side, row in team_stats_by_side.items():
        ...
        stat = LolTeamGameStat(
            ...
            patch=patch,
            team_name=team_name,
            opponent_name=blue_team if side == "red" else red_team,
            side=side,
            result=_int(row.get("result")),
            kills=_int(row.get("kills")),
            deaths=_int(row.get("deaths")),
            assists=_int(row.get("assists")),
            team_kills=_int(row.get("teamkills")),
            team_deaths=_int(row.get("teamdeaths")),
            dragons=_int(row.get("dragons", row.get("elementaldrakes"))),
            barons=_int(row.get("barons")),
            towers=_int(row.get("towers")),
            inhibitors=_int(row.get("inhibitors")),
            game_length_seconds=game_length,
            first_blood=_bool(row.get("firstblood")),
            first_tower=_bool(row.get("firsttower")),
            gold=_int(row.get("totalgold", row.get("gold"))),
            final_gold=_int(row.get("totalgold", row.get("gold"))),
    ...
    for row in player_rows:
        ...
        session.add(
            LolPlayerGameStat(
                ...
                role=(row.get("position") or row.get("role") or "").strip(),
                champion=(row.get("champion") or "").strip(),
                kills=_int(row.get("kills")),
                deaths=_int(row.get("deaths")),
                assists=_int(row.get("assists")),
                cs=_int(row.get("totalcs", row.get("cs"))),
